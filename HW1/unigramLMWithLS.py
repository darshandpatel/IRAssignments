    ...
    for doc_id in final_doc_ids:
        ...
        for term in terms:
            if doc_id in tf_terms_per_doc[term]:
                p_laplace = math.log((tf_terms_per_doc[term][doc_id] + 1.0) / (float(docs_length[doc_id] + unique_terms)))
            else:
                p_laplace = math.log(1.0 / (float(docs_length[doc_id] + unique_terms)))
            score += p_laplace
        unigram_lm_ls[doc_id]=round(score,2)

    
    #sorted_doc = OrderedDict(sorted(doc_okapi_tf_score.items(), key=lambda t: t[1],reverse=True))
    sorted_doc = sorted(unigram_lm_ls.iteritems(), key=lambda x:-x[1])[:1000]
    query_outputs[no] = sorted_doc
#query_outputs = collections.OrderedDict(sorted(query_outputs.items()))
file = open("/Users/Pramukh/Documents/Information Retrieval Data/AP_DATA/resultunigramLMWithLS.txt", "w")
